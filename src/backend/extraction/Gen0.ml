...
let rec cgraph_statement s nd nret nrev nbrk =
  ...
  | Sloop sbody ->
    bind (Obj.magic coq_Monad_mon) reserve_instr (fun n1 ->
      bind (Obj.magic coq_Monad_mon)
        ...
        bind (Obj.magic coq_Monad_mon)
          (Obj.magic update_instr n1 (Language1.Sskip n2)) (fun _ ->
  ...
  | Slog (topics, args) -> add_instr (Language1.Slog (topics, args, nd))
...
let cgraph_function f =
  let cgraph_fun =
    bind (Obj.magic coq_Monad_mon) (add_instr Sdone) (fun nret ->
      bind (Obj.magic coq_Monad_mon) (add_instr Language1.Srevert)
  ...
  (match cgraph_fun init_state with
   | Fail ->
     Error (String ((Ascii (Coq_true, Coq_true, Coq_false, Coq_false,
       ...
       EmptyString))))))))))))))))))))))))))))))))))))))))))))))))))))))
   | OK (nentry, s) ->
     ret (Obj.magic coq_Monad_optErr) { Language1.fn_return = f.fn_return;
       Language1.fn_params = f.fn_params; Language1.fn_temps = f.fn_temps;
       fn_code = s.st_code; fn_entrypoint = nentry })
