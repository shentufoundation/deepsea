...
Require Import BinInt.  (* Z_scope, (_ | _) *)  (* Actually this changed in 8.7, need to fix. *)
...
Section HYPER_TYPE.
  ...
  *)
  ...
  (** [HyperBuiltin] exposes the Ethereum EVM builtin operations. *)
  ...
  Class HyperUnaryPassthrough op (tp tpo : type_pair)
     `{hti : HyperTypeImpl tp, htio : HyperTypeImpl tpo, !HyperUnaryImpl op tp tpo}
     : Prop := {
    (* Hunary_inject : forall j a b, ht_inject j a b -> ht_inject j (Hunary a) (Hunary b) *)
  }.
...
Section HYPER_CONSTRUCTOR.
  ...
  Proof.
    induction fld_ids; intros.
    - destruct fld_tps; try discriminate.
      injection H as <-.
      reflexivity.
    - destruct fld_tps; try discriminate.
      simpl in H.
      destruct (compose_fieldlist fld_ids fld_tps) eqn:H0; try discriminate.
      pose proof (IHfld_ids _ _ H0).
      injection H as <-.
  ...
  Fixpoint compose_cstruct_val map fld_ids {fld_tps}
    ...
    | HCons htp _ f res => match fld_ids with
      | nil => map  (* Impossible, but let's not bother *)
      | fld_id :: res_ids =>
...
Section HYPER_LTYPE.
  ...
  Record ltype_pair tp := {
    ltype_tp_marker : type_marker tp;   (* Hack to be able to use record syntax {| |}. Otherwise there would be no way to provide the value of tp to Coq's type checker. *)

    ltype_get : GetHighData -> unpair_ft tp;                   (* Lens with the abstract data type *)
    ltype_set : unpair_ft tp -> GetHighData -> GetHighData;

    ltype_set_ocond : OProp1 GetHighData;                      (* These verification conditions ensure we do not read "bad" data from memory. *)
    ltype_get_extra_ocond : OProp1 GetHighData;                (*   E.g. in user datatypes, some branches are marked "default", but we do not want to read/write those. *)

    ltype_ghost : bool;     (* Should we synthesize C code for accessing this field or not? *)
    ltype_ident : ident_ext;    (* If so, then this is the corresponding memory ident *)
  }.
  ...
  Class HyperLTypeDir {tp}`{HyperTypeImpl tp} (i : ident_ext) 
    ...
    (* If the memory m matches the abstract data d, then reading out a part of d will match a part of m. See "extended C values" in the dissertation. *)
    ...
    ltype_set_match : forall f j d m, ht_ft_cond f ->
      ...
      forall m',  (* [m'] represent post-Mem.store (potentially partial) [m] *)
        (forall i',  
  ...
  (* Those validity properties that don't mention the memory. *)
  Class HyperLTypeStatic {tp}(lt : ltype_pair tp) (new_glbl : list ident) : Prop := {
  }.

  (* This class combines both the "direct" and the static correctness conditions. *)
  ...
  Lemma cval_match_indirect_by_value_load `{hbvt : HyperByValueType} m i cv:
      ...
      exists v, IdentExtMap.get i m = EEVal v
             /\ cval_match v cv.
  Proof.
    assert (by_ref := ht_by_value_access_mode).
    inversion 1; subst.
    - exists v.
      split; auto; assumption.
    - rewrite <- H0 in by_ref; inversion by_ref. (* discriminate. *)
    - rewrite <- H0 in by_ref; inversion by_ref. (* discriminate. *)
    - rewrite <- H0 in by_ref; inversion by_ref. (* discriminate. *)
  Qed.
  ...
  Proof.
    assert (by_ref := ht_by_value_access_mode).
    intros.
    eapply CVMIval; eassumption.
  Qed.

  (* The ltype_pair lenses always start from the full abstract datatype.
     The following lenses represent a "smaller" mapping, from a struct to one of its fields (HyperFieldImpl)
     or from an array to one of its entries (HypeIndexImpl).

     In HyperTypeInst.v, we combine these with ltype_pair to derive  smaller ltype_pair's.
 *)
  ...
  (* This is used as a precondition to some lemmas about passthrough methods. *)
