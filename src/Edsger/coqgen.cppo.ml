...
let output_hyper_type_impl out t =
  ...
  let _ = match t.aTypeDesc, t.aTypeProvable with
    | ATarray (n, t'), _ ->       
      if need_injection t' then
        output_string out ("
  ZMap_based_array_hyper_type_impl " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
      else
        output_string out ("
  ZMap_inject_equal_array_hyper_type_impl " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
   ...
   Int256Tree_based_hashmap_hyper_type_impl " ^ t'.aTypePairIdent ^ ".\n")
        ...
        output_string out ("
   Int256Tree_inject_equal_hashmap_hyper_type_impl " ^ t'.aTypePairIdent ^ ".\n")
    | ATmapping _, _ ->
...
Proof. solve_algebraic_record_data_type_hyper_type " ^ t.aTypePairIdent ^
  ...
  ZMap_based_array_hyper_type " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
        ...
        output_string out (" :=
  ZMap_inject_equal_array_hyper_type " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
    | ATmapping (t1, t2)  ->
       ...
       else if need_injection t2 then
         output_string out (" :=
  Int256Tree_based_hashmap_hyper_type " ^ t2.aTypePairIdent ^ ".\n")
      else
        output_string out (" :=
  Int256Tree_inject_equal_hashmap_hyper_type " ^ t2.aTypePairIdent ^ ".\n")
    | ATlist t' ->
      coqgen_fatal_error __LOC__ "output_hyper_type"
        ("list type " ^ string_of_a_type false t ^ " marked as provable")
    | ATexternal extype ->
...
let output_hyper_index_impl out array_type_pair_ident n t' =
  ...
  ZMap_based_array_hyper_index_impl " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
    ...
    output_string out ("
  ZMap_inject_equal_array_hyper_index_impl " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
  (*
  output_string out ("Global Instance " ^ array_type_pair_ident ^
...
let output_hyper_index out array_t n t' =
  ...
  ZMap_based_array_hyper_index " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
      ...
      output_string out ("
  ZMap_inject_equal_array_hyper_index " ^ t'.aTypePairIdent ^ " "
			 ^ string_of_int n ^ ".\n")
...
let output_hyper_hash_impl out type_pair_ident t1 t2 =
  ...
  else if need_injection t1 then
    output_string out ("
  Int256Tree_based_hashmap_hash_impl " ^ t2.aTypePairIdent ^ ".\n")
  else
    output_string out ("
  Int256Tree_inject_equal_hashmap_hash_impl " ^ t2.aTypePairIdent ^ ".\n")
...
let output_hyper_hash out type_pair_ident t1 t2 =
  ...
  if not(t1.aTypeDesc = ATbuiltin Tuint || t1.aTypeDesc = ATbuiltin Taddress) then
    ...
    output_string out ("
  Int256Tree_based_hashmap_hash " ^ t2.aTypePairIdent ^ " (fun j a b => iff_refl _).\n")
  else
    output_string out ("
  Int256Tree_inject_equal_hashmap_hash " ^ t2.aTypePairIdent ^ ".\n")
...
let output_hyper_index_passthrough out array_type_pair_ident n t' =
  ...
  ZMap_based_array_hyper_index_passthrough " ^ t'.aTypePairIdent ^ " " ^ string_of_int n ^ ".\n")
    ...
    output_string out ("
  ZMap_inject_equal_array_hyper_index_passthrough " ^ t'.aTypePairIdent ^ " " ^
      string_of_int n ^ " (fun j a b => iff_refl _).\n")
  (*
  output_string out ("Global Instance " ^ array_type_pair_ident ^
    "_index_passthrough : HyperIndexPassthrough " ^ array_type_pair_ident ^ " " ^
    t'.aTypePairIdent ^ ".
...
let rec output_lexpr out obj ind e = match e.aLexprDesc with
  | AEvar i -> output_string out ("(LCvar " ^ obj ^ "_" ^ i ^ "_var)")
  | AEfield (e', f) ->
  ...
  | AEindex (e', idx) ->
    ...
    output_char out ')'
  | AEhash (e', idx) ->
    output_string out ("(LChash " 
			^ e.aLexprType.aTypePairIdent ^ "\n" ^ ind);
    output_lexpr out obj ("  " ^ ind) e';
    output_string out ("\n" ^ ind);
    output_rexpr out ("  " ^ ind) idx;
    output_char out ')'
...
let output_lexpr_get_gallina out obj se d =
  let rec output ind (e : Ast.a_lexpr) = match e.aLexprDesc with
    | AEvar v ->
      output_string out ("(" ^ obj ^ "_" ^ v ^ "_var.(ltype_get) " ^ d ^ ")")
    | AEfield (e', fld) ->
...
let output_lexpr_set_gallina out obj se v d =
  let rec output out_v ind e = match e.aLexprDesc with
    | AEvar var ->
      output_string out (obj ^ "_" ^ var ^ "_var.(ltype_set) ");
      out_v ();
      output_string out (" " ^ d);
    | AEfield (e', fld) ->
    ...
    | AEhash (e', idx) ->
      output
        (fun _ ->
          output_string out ("(@Hhash_set _ _ " ^
            e'.aLexprType.aTypePairIdent ^ "_index_impl ");
          output_rexpr_gallina out se ("    " ^ ind) idx;
          output_char out ' ';
          out_v ();
...
let prepare_command env out base_layer obj method_full_name =
  ...
  Eval cbv -[Int256.repr Int256.modulus zeq zle zlt Z.iter Z.le Z.lt Z.gt Z.ge Z.eqb Z.leb Z.ltb Z.geb Z.gtb Z.mul Z.div Z.modulo Z.add Z.sub
             ret bind mzero get put modify
             GetHighData is_true bool_dec ZMap.get ZMap.set  Int256Tree.get Int256Tree.set SpecTree.get
             " ^ env.folded_external_symbol_string ^ "
             " ^ !(env.folded_symbols) ^ "]
...
"     reflexivity.
  ...
  Eval cbv -[" ^ cbv_ignore_base ^ "
             ret bind mzero get put gets guard modify
             GetHighData is_true bool_dec ZMap.get ZMap.set Int256Tree.get Int256Tree.set hlist_hd
             " ^ env.folded_external_symbol_string ^ "
             " ^ !(env.folded_symbols) ^ "]
...
Existing Instances " ^ l.aLayerName ^ "_overlay_spec.\n"))
        (Str.regexp ("\\bLemma +" ^ o.aObjectName ^ "_\\(.+\\)_spec_requires_kernel_mode\\b"))
        file_class_ObjAux in
#endif
    let code =
#ifdef REDACTED
...
Context`{" ^ l.aLayerName ^ "_pres_inv : !" ^ l.aLayerName ^ "_preserves_invariants}.\n");
    ...
    iter_nonghost_methods (fun m ->
      ...
      if not (Hashtbl.mem prf_tbl m.aMethodName) then begin
        let lemma_name =
          "Lemma " ^ method_full_name ^ "_spec_requires_kernel_mode" in
        Hashtbl.add prf_tbl m.aMethodName lemma_name;
        output_string proofs ("\n" ^ lemma_name ^ args ^ " d" ^ retarg ^ " :
    " ^ method_full_name ^ "_spec" ^ args ^ " d = Some " ^ ret ^ " ->
    kernel_mode d.
Proof.
  intros spec.
  unfold " ^ method_full_name ^ "_spec in spec.
  simpl.
...
(*  - apply semof_prim_kernel_mode_" ^ string_of_int nargs ^ suffix ^ ".
    + exact relate_AbData_translates_kernel_mode.
    + exact " ^ method_full_name ^ "_spec_requires_kernel_mode. *)
Qed.\n")
    ) o.aObjectMethods;
